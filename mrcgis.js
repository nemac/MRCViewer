(function ($) {
        ...
        this.shareUrl = function() {
            ...
            $.each(this.map.layers, function () {
                ...
                if (! this.isBaseLayer) {
                    ...
                    layerLids.push(this.mrcgisLayer.lid);
                    layerAlphas.push(op);
                }
            });
    ...
    EventEmitter.declare(mrcgis.App);
    ...
    function Layer(settings) {
        ...
        this.createOpenLayersLayer = function() {
            ...
            this.openLayersLayer.mrcgisLayer = this;
            return this.openLayersLayer;
        };
    ...
    mrcgis.init = function() {
        app = new mrcgis.App();
        var shareUrlInfo = ShareUrlInfo.parseUrl(window.location.toString());
        app.launch('config/mrc_config.xml', shareUrlInfo);
        mrcgis.app = app;
    };
            ...
            function (e) {
                ...
                app.map.addPopup(lastPopup =
                                 new OpenLayers.Popup.FramedCloud(
                                     "mrcgisMultigraphPopup", 
                                     coords,
                                     null,
                                     '<div id="mrcgisMultigraphMessage"><img class="ajax-loader-image" src="icons/ajax-loader.gif"/></div><div id="mrcgisMultigraph" style="width: 600px; height: 300px;"></div>',
                                     null,
                                     true));
                var promise = window.multigraph.jQuery('#mrcgisMultigraph').multigraph({
                ...
                window.multigraph.jQuery('#mrcgisMultigraph').multigraph('done', function() {
                    $('#mrcgisMultigraphMessage').empty();
                    $('#mrcgisMultigraphMessage').text(Mustache.render('MODIS NDVI for Lat: {{{lat}}} Lon: {{{lon}}}',
    ...
    // exports, for testing:
    //
    mrcgis.BaseLayer                         = BaseLayer;
    mrcgis.AccordionGroup                    = AccordionGroup;
    mrcgis.AccordionGroupSublist             = AccordionGroupSublist;
    mrcgis.Layer                             = Layer;
    mrcgis.Theme                             = Theme;
    mrcgis.createWMSGetFeatureInfoRequestURL = createWMSGetFeatureInfoRequestURL;
    mrcgis.stringContainsChar                = stringContainsChar;
    mrcgis.ShareUrlInfo                      = ShareUrlInfo;
    window.mrcgis                            = mrcgis;
